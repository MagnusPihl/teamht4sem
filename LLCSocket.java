...
public class LLCSocket extends LinkLayerSocket {                
            ...
            do {
                ...
                if (this.data != -1) {
                    if ((this.inPointer < DATA_OFFSET)) {
                        ...
                        if (this.escape) {                            
                            this.buffer[this.inPointer++] = (byte)this.data; //reverse order
                            this.buffer[this.inPointer++] = (byte)(~this.data);
                            this.escape = false;
                        } else {
                            if (this.data == PACKET_HEADER) { //if new packet start over
                               this.inPointer = 1;
                            }  else if ((byte)this.data == ESCAPE_CHAR) {
                                this.escape = true;
                            } else {
                                this.buffer[this.inPointer++] = (byte)this.data;
                            }
                ...
                } else if ((this.inPointer == 0) || ((int)System.currentTimeMillis() > this.timeout)) {
                    this.inPointer = 0;
                    return false;
                }
    ...
    public class LLCOutputStream extends ClearableOutputStream {
        ...
        public synchronized void write(int buffer) throws IOException {
            if (((byte)buffer == ESCAPE_CHAR)||((byte)buffer == PACKET_HEADER)) {
                this.buffer[this.writePointer++] = (byte)ESCAPE_CHAR;
