        ...
        do {
            ...
            if (available == 1) {
                if ((this.packetIndex < DATA_OFFSET)) {
                    ...
                    if (this.escape) {
                        this.packetBuffer[this.packetIndex++] = data[0]; //reverse order
                        this.packetBuffer[this.packetIndex++] = (byte)(~data[0]);
                        this.escape = false;
                    } else {
                        if (data[0] == PACKET_HEADER) { //new packet
                            this.packetIndex = 1;
                        } else if (data[0] == ESCAPE_CHAR) {
                            this.escape = true;                        
                        } else {
    ...
    public class TowerOutputStream extends ClearableOutputStream {
        ...
        public synchronized void write(int buffer) throws IOException {
            if (((byte)buffer == ESCAPE_CHAR)||((byte)buffer == PACKET_HEADER)) {
                this.packetBuffer[this.packetIndex++] = (byte)ESCAPE_CHAR;
